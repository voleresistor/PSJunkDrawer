...
param( 
    ...
    [string]$SiteServer, 
    #[parameter(Mandatory=$false,HelpMessage="Name of the Software Update Group")] 
    #[string]$SUGName, 
...
Process { 
    try {
        foreach ($sug in (get-wmiobject -Namespace "root\sms\site_$($SiteCode)" -Class SMS_AuthorizationList -ComputerName $SiteServer | select LocalizedDisplayName)){
